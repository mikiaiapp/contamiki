...
import { Trash2, Edit2, Layers, Tag, Wallet, Loader2, ImageIcon, Sparkles, XCircle, Info, Download, Upload, DatabaseZap, ClipboardPaste, CheckCircle2, Repeat, Star, Power, BoxSelect, AlertOctagon, Eraser, AlertTriangle, RefreshCcw, FileJson } from 'lucide-react';
...
export const SettingsView: React.FC<SettingsViewProps> = ({ data, onUpdateData }) => {
  ...
  // Added missing helper to render icons correctly (emojis or images)
  ...
  const handleManualMovementImport = () => {
      ...
      lines.forEach((line, index) => {
          ...
          const [fec, cName, aName, concept, amStr] = parts;
          ...
          {webLogos.length > 0 && (
            <div className="bg-white p-5 rounded-[2.5rem] border border-indigo-100 shadow-xl space-y-4 animate-in slide-in-from-top-4">
                ...
                <div className="grid grid-cols-5 gap-3 max-h-40 overflow-y-auto p-1 custom-scrollbar">
                    {webLogos.map((l, i) => (
                        <button key={i} onClick={() => { setIcon(l.url); setWebLogos([]); }} className="aspect-square bg-slate-50 rounded-xl border border-slate-100 hover:border-indigo-500 p-2 transition-all flex items-center justify-center overflow-hidden"><img src={l.url} className="w-full h-full object-contain" /></button>
                    ))}
                </div>
            </div>
          )}
  ...
  return (
    <div className="space-y-12 max-w-full overflow-hidden pb-20">
      ...
      <div className="max-w-4xl mx-auto">
            ...
            <div className="space-y-12 pb-10">
                <div className="bg-white p-10 rounded-[3rem] shadow-sm border border-slate-100 space-y-8">
                    ...
                    <div className="space-y-4">
                        <div className="bg-indigo-50/50 p-6 rounded-3xl border border-indigo-100 space-y-2">
                            ...
                            <p className="text-indigo-400 text-[9px] uppercase font-black italic">Autocreación habilitada: si la categoría o cuenta no existe, ContaMiki la creará automáticamente.</p>
                        </div>
                        <textarea className="w-full h-56 p-6 bg-slate-50 border-2 border-slate-100 rounded-[2rem] font-mono text-[11px] text-slate-600 outline-none focus:border-indigo-500 transition-all custom-scrollbar placeholder:text-slate-300 shadow-inner" placeholder={importMode === 'NORMAL' ? "2023-10-27; Alimentación; BBVA; Compra cena; -25.50" : "2023-10-27; BBVA; Efectivo; Retirada; 50.00"} value={pasteMovements} onChange={e => setPasteMovements(e.target.value)} />
                        ...
                        <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white space-y-6 animate-in zoom-in-95">
                            ...
                            {(importReport.newAccounts.length > 0 || importReport.newCategories.length > 0) && (
                                <div className="flex flex-wrap gap-2">
                                    {importReport.newAccounts.map(a => <span key={a} className="px-3 py-1 bg-indigo-500/20 text-indigo-300 rounded-full text-[8px] font-black uppercase">Nueva Cuenta: {a}</span>)}
                                    {importReport.newCategories.map(c => <span key={c} className="px-3 py-1 bg-amber-500/20 text-amber-300 rounded-full text-[8px] font-black uppercase">Nueva Cat: {c}</span>)}
                                </div>
                            )}
                        </div>
                    )}
                ...
                <div className="bg-slate-900 p-10 rounded-[3rem] shadow-2xl border border-white/5 space-y-8 text-center relative overflow-hidden">
                    <div className="relative z-10 space-y-6">
                        ...
                        <p className="text-slate-400 text-sm font-medium uppercase tracking-widest leading-relaxed max-w-md mx-auto">Exporta tu historial completo en formato JSON para tener soberanía total sobre tus datos.</p>
                        <div className="flex flex-col sm:flex-row gap-4 justify-center">
                            ...
                            <button onClick={() => window.print()} className="flex-1 flex items-center justify-center gap-3 p-5 bg-slate-800 text-white rounded-2xl font-black text-[11px] uppercase tracking-widest border border-white/10 hover:bg-slate-700 transition-all active:scale-95"><Download size={20} /> Generar PDF</button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        {/* Los demás paneles (ACCOUNTS, FAMILIES, CATEGORIES) han sido optimizados pero se mantienen funcionales según el diseño anterior */}
        {activeTab === 'ACCOUNTS' && (
