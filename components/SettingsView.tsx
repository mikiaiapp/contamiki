...
export const SettingsView: React.FC<SettingsViewProps> = ({ data, books, currentBookId, multiState, onUpdateData, onReplaceFullState, onNavigateToTransactions, onDeleteBook }) => {
  ...
  const renderIcon = (iconStr: string, className = "w-10 h-10") => {
    ...
    if (iconStr.startsWith('http') || iconStr.startsWith('data:image')) {
        return (
            <div className={`${className} rounded-lg overflow-hidden relative bg-[radial-gradient(#e2e8f0_1px,transparent_1px)] [background-size:8px_8px] bg-slate-50 flex items-center justify-center p-1 border border-slate-100 shadow-inner`}>
                <img 
                    src={iconStr} 
                    ...
                    onError={(e) => {
                        ...
                        if (parent) {
                            ...
                            placeholder.className = 'text-[8px] opacity-40';
  ...
  const handleIASearch = async (name: string) => {
    if (!name) { alert("Escribe un nombre."); return; }
    setIsSearchingWeb(true);
    setFailedLogos(new Set());
    setIaSearchStatus("Explorando variantes...");
    try {
        const results = await searchInternetLogos(name, (status) => setIaSearchStatus(status));
        setWebLogos(results);
    } catch (err) {
        console.error(err);
  ...
  const renderIconInput = (icon: string, setIcon: (s: string) => void, currentName: string) => (
    <div className="space-y-4 w-full">
        <div className="flex flex-col sm:flex-row items-center gap-6 bg-slate-50 p-4 sm:p-6 rounded-[2rem] border border-slate-100 shadow-inner">
            <div className="w-16 h-16 sm:w-20 sm:h-20 flex-shrink-0 flex items-center justify-center border-4 border-white rounded-[1.5rem] bg-white overflow-hidden shadow-lg hover:scale-105 transition-transform">
                {renderIcon(icon, "w-12 h-12 sm:w-14 sm:h-14")}
            </div>
            <div className="flex-1 space-y-3 w-full">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-center sm:text-left">Gestión de Identidad</p>
                <div className="flex flex-wrap gap-2 justify-center sm:justify-start">
                    <button 
                        onClick={() => handleIASearch(currentName)} 
                        disabled={isSearchingWeb}
                        className="px-5 py-3 bg-indigo-600 text-white rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-slate-950 transition-all flex items-center gap-2 shadow-lg disabled:opacity-50"
                    >
                        {isSearchingWeb ? <Loader2 size={14} className="animate-spin"/> : <Sparkles size={14}/>} 
                        {isSearchingWeb ? iaSearchStatus : "Búsqueda Ultra"}
                    </button>
        ...
        {visibleWebLogos.length > 0 && (
          <div className="bg-white p-6 rounded-[2.5rem] border-2 border-indigo-100 shadow-2xl space-y-5 animate-in slide-in-from-top-4 z-[300] relative">
              <div className="flex justify-between items-center border-b border-slate-100 pb-4">
                  <div className="flex flex-col">
                    <span className="text-[11px] font-black text-indigo-500 uppercase tracking-widest flex items-center gap-2">
                        <Sparkles size={14}/> Galería de Alternativas ({visibleWebLogos.length})
                    </span>
                    <span className="text-[8px] font-bold text-slate-400 uppercase mt-0.5">La IA ha generado variantes de: {currentName}</span>
                  </div>
                  <button onClick={() => setWebLogos([])} className="text-slate-300 hover:text-rose-500 p-1">
                      <XCircle size={20}/>
                  </button>
              </div>
              <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-8 gap-3 max-h-96 overflow-y-auto p-2 custom-scrollbar">
                  {visibleWebLogos.map((l, i) => (
                    <button 
                        key={i} 
                    ...
                    >
                        <img 
                            ...
                            onError={() => handleImageError(l.url)}
                            referrerPolicy="no-referrer"
                        />
                        <div className="absolute bottom-0 inset-x-0 bg-indigo-600 text-white text-[5px] py-0.5 opacity-0 group-hover:opacity-100 transition-opacity uppercase font-black text-center">
                            {l.source}
                        </div>
                    </button>
                  ))}
              ...
              <div className="bg-slate-50 p-3 rounded-2xl border border-slate-100">
                <p className="text-[8px] text-slate-500 font-bold uppercase text-center tracking-[0.2em] leading-relaxed">
                    Sugerencia: Si no encuentras lo que buscas, prueba con términos similares (ej: "Zapatillas" en lugar de "Zapatos").
                </p>
              </div>
          </div>
        )}
  ...
  const handleBackup = async () => {
      ...
      if (backupScope === 'CURRENT') {
          payload = { version: '1.5.0', scope: 'SINGLE', timestamp: new Date().toISOString(), metadata: currentBookMeta, data: data };
          stats = { type: 'Libro Individual', name: currentBookMeta?.name, transactions: data.transactions.length, attachments: data.transactions.filter(t => t.attachment).length };
      } else {
  ...
  const handleRestoreFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      reader.onload = (evt) => {
          try {
              ...
              if (json.scope === 'ALL') setRestoreModal({ type: 'MULTI', data: json });
              else if (json.scope === 'SINGLE') { setRestoreModal({ type: 'SINGLE', data: json }); setSingleRestoreConfig(prev => ({ ...prev, newName: json.metadata.name + ' (Copia)', newColor: json.metadata.color })); }
              else alert("Formato de backup no reconocido.");
  ...
  const executeRestore = () => {
      ...
      if (restoreModal.type === 'MULTI') {
          ...
          if (singleRestoreConfig.target === 'CURRENT') fullState.booksData[currentBookId] = bookData;
          else if (singleRestoreConfig.target === 'EXISTING') {
              if (!singleRestoreConfig.targetId) { alert("Por favor, selecciona un libro de destino."); return; }
              fullState.booksData[singleRestoreConfig.targetId] = bookData;
          } else {
              const newId = generateId();
              fullState.booksMetadata.push({ id: newId, name: singleRestoreConfig.newName || 'Nuevo Libro', color: singleRestoreConfig.newColor || 'BLACK', currency: 'EUR' });
              fullState.booksData[newId] = bookData;
              fullState.currentBookId = newId;
          }
          onReplaceFullState(fullState);
          finalStats = { type: 'RESTAURACIÓN INDIVIDUAL', result: 'Completada', detail: `Libro "${singleRestoreConfig.target === 'NEW' ? singleRestoreConfig.newName : books.find(b => b.id === (singleRestoreConfig.targetId || currentBookId))?.name}" actualizado.` };
      }
                  ...
                  <div className="space-y-8">
                      {restoreModal.type === 'MULTI' ? (
                          <div className="bg-rose-50 p-6 rounded-[2rem] border border-rose-100 text-center space-y-4">
                              ...
                              <p className="text-xs text-rose-700 font-medium leading-relaxed">Se eliminarán todos tus libros actuales y se sustituirán por los de la copia de seguridad.</p>
                          </div>
                      ) : (
                          <div className="space-y-6">
                      ...
                      <button onClick={executeRestore} className="w-full py-6 bg-slate-950 text-white rounded-2xl font-black uppercase text-[12px] tracking-[0.2em] shadow-2xl hover:bg-emerald-600 transition-all">Confirmar y Restaurar</button>
                  </div>
              </div>
          </div>
      )}
