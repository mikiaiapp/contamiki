...
const App: React.FC = () => {
  ...
  const handleCreateBookFromImport = (importData: AppState, name: string, color: BookColor = 'BLUE') => {
      ...
      const newState = {
          ...multiState,
          booksMetadata: [...multiState.booksMetadata, newBook],
          booksData: { ...multiState.booksData, [newId]: importData },
          currentBookId: newId
      };
      ...
      setTimeout(() => alert(`Nueva contabilidad "${name}" creada correctamente.`), 500);
  };

  const handleRestoreToBook = (bookId: string, data: AppState) => {
      const newState = {
          ...multiState,
          booksData: { ...multiState.booksData, [bookId]: data },
      ...
      performSave(newState);
      setCurrentView('RESUMEN');
      setTimeout(() => alert("Copia de seguridad restaurada correctamente en el libro destino."), 500);
  };
  ...
  return (
    ...
    >
      ...
      {currentView === 'SETTINGS' && <SettingsView data={currentAppData} books={multiState.booksMetadata} onUpdateData={updateCurrentBookData} onCreateBookFromImport={handleCreateBookFromImport} onRestoreToBook={handleRestoreToBook} onExportData={handleExportData} />}
